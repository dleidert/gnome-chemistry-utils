...
static void gnome_canvas_shape_ext_class_init   (GnomeCanvasShapeExtClass *class);
static void gnome_canvas_shape_ext_init         (GnomeCanvasShapeExt     *shape);
static void gnome_canvas_shape_ext_destroy      (GtkObject               *object);
static void gnome_canvas_shape_ext_set_property (GObject               *object,
					     guint                  param_id,
					     const GValue          *value,
                                             GParamSpec            *pspec);
static void gnome_canvas_shape_ext_get_property (GObject               *object,
					     guint                  param_id,
					     GValue                *value,
                                             GParamSpec            *pspec);

static void   gnome_canvas_shape_ext_update      (GnomeCanvasItem *item, double *affine, ArtSVP *clip_path, int flags);
static void   gnome_canvas_shape_ext_realize     (GnomeCanvasItem *item);
static void   gnome_canvas_shape_ext_unrealize   (GnomeCanvasItem *item);
static void   gnome_canvas_shape_ext_draw        (GnomeCanvasItem *item, GdkDrawable *drawable,
                                              int x, int y, int width, int height);
static double gnome_canvas_shape_ext_point       (GnomeCanvasItem *item, double x, double y,
                                              int cx, int cy, GnomeCanvasItem **actual_item);
static void   gnome_canvas_shape_ext_render      (GnomeCanvasItem *item, GnomeCanvasBuf *buf);
static void   gnome_canvas_shape_ext_bounds      (GnomeCanvasItem *item,
...
static void gcbp_ensure_gdk (GnomeCanvasShapeExt * bpath);
static void gcbp_destroy_gdk (GnomeCanvasShapeExt * bpath);
...
GType
gnome_canvas_shape_ext_get_type (void)
{
	static GType shape_ext_type;

	if (!shape_ext_type) {
		static const GTypeInfo object_info = {
			sizeof (GnomeCanvasShapeExtClass),
			(GBaseInitFunc) NULL,
			(GBaseFinalizeFunc) NULL,
			(GClassInitFunc) gnome_canvas_shape_ext_class_init,
			(GClassFinalizeFunc) NULL,
			NULL,			/* class_data */
			sizeof (GnomeCanvasShapeExt),
			0,			/* n_preallocs */
			(GInstanceInitFunc) gnome_canvas_shape_ext_init,
			NULL			/* value_table */
		};

		static const GInterfaceInfo print_info = {
			(GInterfaceInitFunc) gnome_canvas_shape_ext_print_init,
			NULL, NULL
		};

		shape_ext_type = g_type_register_static (GNOME_TYPE_CANVAS_ITEM, "GnomeCanvasShapeExt",
						    &object_info, 0);

		g_type_add_interface_static (shape_ext_type, G_TYPE_PRINTABLE, &print_info);
	}

	return shape_ext_type;
}

static void
gnome_canvas_shape_ext_class_init (GnomeCanvasShapeExtClass *class)
{
	...
	gobject_class->set_property = gnome_canvas_shape_ext_set_property;
	gobject_class->get_property = gnome_canvas_shape_ext_get_property;
	...
	object_class->destroy = gnome_canvas_shape_ext_destroy;

	item_class->update = gnome_canvas_shape_ext_update;
	item_class->realize = gnome_canvas_shape_ext_realize;
	item_class->unrealize = gnome_canvas_shape_ext_unrealize;
	item_class->draw = gnome_canvas_shape_ext_draw;
	item_class->point = gnome_canvas_shape_ext_point;
	item_class->render = gnome_canvas_shape_ext_render;
	item_class->bounds = gnome_canvas_shape_ext_bounds;
}

static void
gnome_canvas_shape_ext_init (GnomeCanvasShapeExt *shape)
{
...
static void
gnome_canvas_shape_ext_destroy (GtkObject *object)
{
	GnomeCanvasShapeExt *shape;
	GnomeCanvasShapePriv *priv;

	g_return_if_fail (object != NULL);
	g_return_if_fail (GNOME_IS_CANVAS_SHAPE_EXT (object));
...
void
gnome_canvas_shape_ext_set_path_def (GnomeCanvasShapeExt *shape, GnomeCanvasPathDef *def) 
{
	GnomeCanvasShapePriv *priv;

	g_return_if_fail (shape != NULL);
	g_return_if_fail (GNOME_IS_CANVAS_SHAPE_EXT (shape));
...
static void
gnome_canvas_shape_ext_set_property (GObject      *object,
                                 guint         param_id,
                                 const GValue *value,
                                 GParamSpec   *pspec)
{
...
static void
get_color_value (GnomeCanvasShapeExt *shape, gulong pixel, GValue *value)
{
...
GnomeCanvasPathDef *
gnome_canvas_shape_ext_get_path_def (GnomeCanvasShapeExt *shape)
{
	GnomeCanvasShapePriv *priv;
	
	g_return_val_if_fail (shape != NULL, NULL);
	g_return_val_if_fail (GNOME_IS_CANVAS_SHAPE_EXT (shape), NULL);
...
static void
gnome_canvas_shape_ext_get_property (GObject     *object,
                                 guint        param_id,
                                 GValue      *value,
                                 GParamSpec  *pspec)
{
	GnomeCanvasItem         *item = GNOME_CANVAS_ITEM (object);
	GnomeCanvasShapeExt        *shape = GNOME_CANVAS_SHAPE_EXT (object);
...
static void
gnome_canvas_shape_ext_realize (GnomeCanvasItem *item)
{
	GnomeCanvasShapeExt *shape;

	shape = GNOME_CANVAS_SHAPE_EXT (item);
...
static void
gnome_canvas_shape_ext_unrealize (GnomeCanvasItem *item)
{
	GnomeCanvasShapeExt *shape;
...
static void
gnome_canvas_shape_ext_render (GnomeCanvasItem *item,
			     GnomeCanvasBuf *buf)
{
	GnomeCanvasShapeExt *shape;
...
static void
gnome_canvas_shape_ext_draw (GnomeCanvasItem *item,
...
static void
gnome_canvas_shape_ext_ensure_gdk_points (GnomeCanvasShapePrivGdk *gdk, gint num)
{
...
static void
gnome_canvas_shape_ext_update_gdk (GnomeCanvasShapeExt * shape, double * affine, ArtSVP * clip, int flags)
{
	GnomeCanvasShapePriv * priv;
	GnomeCanvasShapePrivGdk * gdk;
	...
	if (priv->path) {
		...
		while (clist) {
			...
			gnome_canvas_shape_ext_ensure_gdk_points (gdk, len);
			for (i = 0; i < len; i++) {
...
static void
gnome_canvas_shape_ext_update (GnomeCanvasItem *item, double *affine, ArtSVP *clip_path, int flags)
{
	GnomeCanvasShapeExt * shape;
	GnomeCanvasShapePriv * priv;
	ArtSVP * svp;
	...
	if (!item->canvas->aa) {
		gnome_canvas_shape_ext_update_gdk (shape, affine, clip_path, flags);
	}
}

static double
gnome_canvas_shape_ext_point (GnomeCanvasItem *item, double x, double y,
			    int cx, int cy, GnomeCanvasItem **actual_item)
{
	GnomeCanvasShapeExt *shape;
	double dist;
...
static void
gcbp_ensure_gdk (GnomeCanvasShapeExt * shape)
{
	g_assert (!((GnomeCanvasItem *) shape)->canvas->aa);
...
static void
gcbp_destroy_gdk (GnomeCanvasShapeExt * shape)
{
	GnomeCanvasShapePrivGdk * gdk;
...
static void
gcbp_ensure_mask (GnomeCanvasShapeExt * shape, gint width, gint height)
{
	GnomeCanvasShapePrivGdk * gdk;
	GCBPDrawCtx * ctx;
...
static void
gnome_canvas_shape_ext_bounds (GnomeCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
	GnomeCanvasShapeExt * shape;
	GnomeCanvasShapePriv * priv;
	ArtDRect bbox;
...
static void
gnome_canvas_shape_ext_export_svg (GPrintable *printable, xmlDocPtr doc, xmlNodePtr node)
{
	GnomeCanvasShapeExt *shape;
	GnomeCanvasShapePriv * priv;
	GnomeCanvasPathDef * path;
	...
	for ( ; bpath->code != ART_END ; bpath++)
		...
		case ART_MOVETO_OPEN :
			...
			break;
		case ART_LINETO :
			g_string_append_printf (string, "L%g %g", bpath->x3, bpath->y3);
			break;
		default :
			break;
		}
	...
	if (priv->fill_set) {
		buf = g_strdup_printf ("#%06x", priv->fill_rgba >> 8);
		xmlNewProp (child, (const xmlChar*)"fill", (const xmlChar*)buf);
		g_free (buf);
		opacity = priv->fill_rgba & 0xff;
		if (opacity != 255) {
	...
	if (priv->outline_set) {
		buf = g_strdup_printf ("#%06x", priv->outline_rgba >> 8);
		xmlNewProp (child, (const xmlChar*)"stroke", (const xmlChar*)buf);
		g_free (buf);
		opacity = priv->outline_rgba & 0xff;
		if (opacity != 255) {
			buf = g_strdup_printf ("%g", (double) opacity / 255.);
			xmlNewProp (child, (const xmlChar*)"sroke-opacity", (const xmlChar*)buf);
			g_free (buf);
		}
		if (priv->width_pixels)
			width = (double) priv->width / shape->item.canvas->pixels_per_unit;
	...
	if (priv->dash.n_dash) {
		char **inc = g_new0 (char*, priv->dash.n_dash + 1);
		buf = g_strdup_printf ("%g", priv->dash.dash[0]);
		for (i = 0; i < priv->dash.n_dash; i++)
			inc[i] = g_strdup_printf ("%g", priv->dash.dash[0]);
		buf = g_strjoinv (",", inc);
		for (i = 0; i < priv->dash.n_dash; i++)
			g_free (inc[i]);
		g_free (inc);
		xmlNewProp (child, (const xmlChar*)"stroke-dasharray", (const xmlChar*)buf);
		g_free (buf);
		buf = g_strdup_printf ("%g", priv->dash.offset);
